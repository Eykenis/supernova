    ...
    public sealed class SpawnShelterWeaponPickupTests
    {
        ...
        public void IsolatedSession_IgnoresPersistentOwnershipAndLoadout()
        {
            ...
            try
            {
                ...
                PlayerPrefs.SetInt(slotKey, (int)PlayerInventoryItem.SolidGun);
                ...
                Assert.That(
                    ...
                    Is.EqualTo((int)PlayerInventoryItem.SolidGun));
            }
            finally
            {
                if (player != null)
        ...
        public void WeaponPickup_AddsWeaponToFirstEmptySlotWithoutPlayerPrefs()
        {
            ...
            try
            {
                ...
                pickupObject = new GameObject("Solid Gun Pickup");
                WeaponPickup pickup = pickupObject.AddComponent<WeaponPickup>();
                pickup.Configure(solidGun, null, null);

                Assert.That(pickup.TryCollect(controller), Is.True);
                Assert.That(controller.OwnsItem(PlayerInventoryItem.SolidGun), Is.True);
                Assert.That(
        ...
        [Test]
        public void SpawnShelterScene_HasIsolatedPlayerAndThreeWeaponPickups()
        {
            ...
            try
            {
                ...
                Vector3Int markerVoxel = roomMarker != null
                    ? testWorld.WorldPositionToVoxel(roomMarker.transform.position)
                    : new Vector3Int(
                        structure.Size.x / 2,
                        structure.Size.y - 2,
                        structure.Size.z - 2);
                Assert.That(
                    pickups.All(pickup =>
                    {
                        ...
                        bool isInsideStructure = pickupVoxel.x > 0
                            && pickupVoxel.x < structure.Size.x - 1;
                        bool isBesideMarker = roomMarker == null
                            || (Mathf.Abs(pickupVoxel.z - markerVoxel.z) <= 6
                                && Mathf.Abs(pickupVoxel.y - markerVoxel.y) <= 6);
                        return isInsideStructure && isBesideMarker;
                    }),
                    Is.True,
                    "Pickups must use the test world's voxel scale and remain "
                    + "inside the authored shelter.");
            }
            finally
            {
                if (openedForTest)
