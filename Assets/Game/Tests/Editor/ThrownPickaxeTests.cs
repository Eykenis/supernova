...
namespace Supernova.Tests
{
    public sealed class ThrownPickaxeTests
    {
        ...
        public void RecallThrow_CallsAThrownPickaxeHome()
        {
            ...
            SoundEffectCue throwSound =
                ScriptableObject.CreateInstance<SoundEffectCue>();
            assets.Add(throwSound);
            SetPrivateField(
                throwController,
                "activeThrowSound",
                throwSound);
            ...
            Vector3 recallPosition = pickaxe.transform.position;
            ...
            Assert.That(received.Cue, Is.SameAs(throwSound));
            Assert.That(received.Position, Is.EqualTo(recallPosition));
            Assert.That(
        ...
        [Test]
        public void FirstThrownTerrainPin_BroadcastsImpactSoundOnce()
        {
            ThrownPickaxe pickaxe = CreatePickaxe(out _, out _);
            SoundEffectCue cue =
                ScriptableObject.CreateInstance<SoundEffectCue>();
            assets.Add(cue);
            Vector3 contactPoint = new Vector3(2f, 3f, 4f);
            int soundRequestCount = 0;
            SoundEffectPlaybackRequest received = default;
            System.Action<SoundEffectPlaybackRequest> observer = request =>
            {
            ...
            Assert.That(received.Position, Is.EqualTo(contactPoint));
        }

        [Test]
        public void ReleasingPickaxeThrow_BroadcastsThrowCueAfterLaunchOnly()
        {
            GameObject playerObject = Create("Player");
            ...
            SoundEffectEvents.PlaybackRequested += observer;
            ThrownPickaxe thrown = null;
            try
            {
                Assert.That(throwController.ReleaseThrow(), Is.Null);
                Assert.That(soundRequestCount, Is.Zero);

                Assert.That(
                    throwController.BeginAim(pickaxeDefinition),
                    Is.True);
                thrown = throwController.ReleaseThrow();
                Assert.That(thrown, Is.Not.Null);
                objects.Add(thrown.gameObject);
            }
            finally
            {
                SoundEffectEvents.PlaybackRequested -= observer;
            ...
            Assert.That(soundRequestCount, Is.EqualTo(1));
            Assert.That(received.Cue, Is.SameAs(pickaxeDefinition.ThrowSound));
            Assert.That(received.Position, Is.EqualTo(thrown.transform.position));
            Assert.That(
                received.VolumeScale,
                Is.EqualTo(0.5f).Within(0.0001f));
            Assert.That(thrown.IsFlying, Is.True);
            Assert.That(toolController, Is.Not.Null);
        }
        ...
        public void PickaxeTool_KeepsTheLoopingMagnetHoldPoseForTheHandAnimation()
        {
            ...
            Assert.That(
                definition.MagnetHoldLoopEndNormalized,
                Is.EqualTo(1f).Within(0.0001f));
            Assert.That(
                definition.MagnetHoldLoopBlendNormalized,
