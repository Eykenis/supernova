...
namespace Supernova.Gameplay
{
    ...
    public sealed class PlayerToolDefinition : ScriptableObject
    {
        ...
        [Tooltip("Upper-body pose used by the magnet. The intro plays once, then only the configured tail segment loops.")]
        [SerializeField] private AnimationClip magnetHoldAnimation;
        [Tooltip("Normalized point where the magnet pose tail loop begins after the intro has played once.")]
        [SerializeField, Range(0f, 0.95f)]
        private float magnetHoldLoopStartNormalized = 0.7f;
        [Tooltip("Normalized point where the magnet pose tail jumps back to the loop start.")]
