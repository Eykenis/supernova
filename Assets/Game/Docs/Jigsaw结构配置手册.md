...
红色 Error 必须清零，黄色 Warning 逐条判断是否符合预期。常见提示见第 6 节。
...
菜单 `Tools > Supernova > World Generation > Create Default Jigsaw Structures`
会重建 `AbandonedMineshaft` 与 `Fortress`。

⚠️ **它会覆盖这两个资产的全部手工调整**。如果你在 Inspector 里改过它们，
不要点这个菜单。它只适合从零恢复默认内容，或作为配置写法的参考实现
（源码见 `Editor/WorldGeneration/JigsawStructureAssetBuilder.cs`）。

## 9. 完整字段参考

### 9.1 结构级（JigsawStructureFeatureDefinition）
...
### 9.3 Socket 级（JigsawConnectorDefinition）
...
**Q：必达房间经常不出现。**
A：见 7.3 节。优先调大 `Layout Attempts` 和 `Max Horizontal Distance`。
