    ...
    public partial class TextMeshProUGUI
    {
        ...
        internal override int SetArraySizes(UnicodeChar[] unicodeChars)
        {
            ...
            if (m_enableAdvanceTextProcess)
                characterCount = SetArraySizesWithATP(unicodeChars);
            else
#endif
                characterCount = SetArraySizesWithoutATP(unicodeChars);
            k_SetArraySizesMarker.End();
            return characterCount;
        }

        internal int SetArraySizesWithoutATP(UnicodeChar[] unicodeChars)
        {
            int spriteCount = 0;
        ...
        protected virtual void GenerateTextMesh()
        {
            ...
            for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
            {
                ...
                if (m_enableAdvanceTextProcess && m_cached_TextElement.harfbuzzSuccess)
                {
                    currentGlyphMetrics = m_cached_TextElement.atpMetrics;
                    currentGlyphMetrics.horizontalBearingX += m_cached_TextElement.m_Glyph.metrics.horizontalBearingX;
                    currentGlyphMetrics.horizontalBearingY += m_cached_TextElement.m_Glyph.metrics.horizontalBearingY;
                }
